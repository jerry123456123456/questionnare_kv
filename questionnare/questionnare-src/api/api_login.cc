...
static int CacheSetCount(const std::string& key, int64_t count) {
    // 1. 严格按协议生成命令（RSET key value），确保参数用空格正确分隔
    char command[1024];
    int command_length = snprintf(command, sizeof(command), "RSET %s %lld\n", key.c_str(), count);
    
    // 2. 缓冲区溢出保护（命令长度校验）
    if (command_length < 0 || command_length >= sizeof(command)) {
        LogError("【CacheSetCount】命令缓冲区溢出！Key: %s, Count: %lld", key.c_str(), count);
        return -1;
    }
    ...
    // 3. 净化响应内容（移除换行符，避免影响匹配）
    if (response[0] != '\0') {
        char* newline = strchr(response, '\n');
        if (newline) *newline = '\0'; // 移除响应中的换行符（如"OK\r\n"变为"OK"）
    }

    // 4. 添加调试日志（打印完整命令与响应）
    LogError("【CacheSetCount】发送命令: %s，响应: %s", command, response);

    // 5. 校验响应有效性（确保"OK"正确匹配）
    if (ret == 0 && strstr(response, "OK") != nullptr) {
        return 0;
    } else {
        LogError("【CacheSetCount】操作失败！Key: %s, Count: %lld，错误响应: %s", 
                 key.c_str(), count, response);
        return -1;
    }
}
