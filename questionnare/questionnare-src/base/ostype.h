...
/*
1. `uint32_t handle`：这个参数通常用于传递一个标识符或句柄，用来标识或操作某个特定的资源、对象或实体。在回调函数中，`handle` 参数可以用来指示回调函数需要处理的具体对象或资源。

2. `void *pParam`：这个参数通常用于传递额外的参数或数据给回调函数。因为它是一个 `void` 类型的指针，可以指向任意类型的数据。回调函数可以根据需要解释和使用这个参数，以便进行相应的操作。
*/
